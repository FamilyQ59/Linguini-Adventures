...
def LittleLight(x, y, player_pos_x, player_pos_y):
    if (player_pos_x - x)**2 <= 10000:
        if -1000 <= (player_pos_y - y) <= 1000 and (player_pos_y - y)**2 <= 2000:
            return True
    return False
...
class DarkMap:
    ...
    def draw(self, file, blocks, surface, Scroll_x, Scroll_y, Number_of_Blocks, player_pos_x, player_pos_y):
        ...
        for layer in self.map:
            ...
            for tile in layer:
                for i in range(0, len(blocks)):
                    if tile == str(i) and LittleLight(x*32, y*32, player_pos_x, player_pos_y):
                        surface.blit(blocks[i], (x * 32 - Scroll_x, y * 32 - Scroll_y))
                if tile != '-1' and len(self.tile_rects) < Number_of_Blocks:
                    self.tile_rects.append(pygame.Rect(x * 32, y * 32, 32, 32))
                x += 1
